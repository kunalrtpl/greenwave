            ...
            <div class="col-md-12">
                <div class="portlet light">
                    <div class="portlet-title">
                        <div class="caption">
                            <span class="caption-subject font-green-sharp bold uppercase">Sale Return Material</span>
                            ...
                            <table class="table table-striped table-bordered table-hover" id="datatable_ajax">
                                <thead>
                                    ...
                                    <tr role="row" class="filter">
                                        ...
                                        <td><input type="text" class="form-control form-filter input-sm" name="serial_no" value="SRM-" ></td>
                                        <td><input type="text" class="form-control form-filter input-sm" name="name" placeholder="Product Name"></td>
                                        <td></td>
                                        <td></td>
                                        <td>
                                            <div class="form-group">
                                                <select class="form-control form-filter input-sm" name="status">
                                                    ...
                                                    @foreach(OSPstatus('view') as $materialStatus)
                                                        <option value="{{$materialStatus}}">{{$materialStatus}}</option>
                                                    @endforeach
                                                    <option value="QC Approved">QC Approved</option>
                                                    <option value="QC Rejected">QC Rejected</option>
                                                    <option value="Packing & Labelling">Packing & Labelling</option>
                                                    <option value="Material Ready for Dispatch">Material Ready for Dispatch</option>
                                                    @else
                                                        @foreach(getOSPAccess('view') as $materialStatus)
                                                        <option value="{{$materialStatus}}">{{$materialStatus}}</option>
                                                    @endforeach
                                                    @endif
                                                </select>
...
<script type="text/javascript">
    window.history.pushState("", "", "/admin/inventory/osp");
</script>
@stop
