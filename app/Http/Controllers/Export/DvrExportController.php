...
class DvrExportController extends Controller
{
    ...
    public function exportUserDvrsCsv()
    {
        $fileName = 'user_dvrs_export_' . date('Ymd_His') . '.csv';

        return new StreamedResponse(function () {
            $handle = fopen('php://output', 'w');

            // ✅ CSV HEADER (Adjusted for user_dvrs schema)
            fputcsv($handle, [
                'DVR ID',
                'Employee Name',
                'Customer Name',
            ...
            DB::table('user_dvrs as ud')
                ->leftJoin('users as u', 'u.id', '=', 'ud.user_id')
                ->leftJoin('customers as c', 'c.id', '=', 'ud.customer_id')
                ->leftJoin('customer_register_requests as crr', 'crr.id', '=', 'ud.customer_register_request_id')
                ->select(
                    'ud.*',
                    'u.name as employee_name',
                    DB::raw("COALESCE(c.name, crr.name) as customer_name")
                ...
                ->chunk(500, function ($rows) use ($handle) {
                    foreach ($rows as $row) {
                        fputcsv($handle, [
                            ...
                            $row->end_lat_long,
                            $row->visit_recorded,
                            $row->purpose_of_visit ?? $row->other_purpose,
                            $row->visit_type,
                            $row->visit_detail,
                            $row->remarks,
                            $row->next_plan,
                            $row->conveyance_type,
                            $row->have_you_met ? 'Yes' : 'No',
                            $row->is_submitted ? 'Yes' : 'No',
                            $row->created_at,
                        ]);
                    }
                });
