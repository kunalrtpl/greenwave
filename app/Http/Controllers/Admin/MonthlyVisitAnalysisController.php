...
/**
 * MonthlyVisitAnalysisController — Admin Module
 * ─────────────────────────────────────────────────────
 * Admin-side screen for the Monthly Visit Analysis report that is
 * otherwise emailed automatically on the 1st of each month by
 * `php artisan report:monthly-visit-analysis`.
 *
 ...
 * MonthlyVisitAnalysisService, so it is identical to the emailed one.
 */
class MonthlyVisitAnalysisController extends Controller
{
    ...
    {
        $title = "Monthly Visit Analysis";
        Session::put('active', 'monthlyVisitAnalysis');
        ...
        return view('admin.reports.monthly_visit_analysis', [
            'title'       => $title,
            'employees'   => MonthlyVisitAnalysisService::eligibleEmployees(),
            'month'       => $month,
            'year'        => $year,
    ...
    public function downloadPdf(Request $request)
    {
        ...
        $employee = MonthlyVisitAnalysisService::eligibleEmployees()
            ->firstWhere('id', (int) $request->user_id);

        if (!$employee) {
            return redirect()
                ->to('admin/monthly-visit-analysis?month=' . $request->month . '&year=' . $request->year)
                ->with('flash_message_error', 'That employee is not part of the Monthly Visit Analysis list.');
        }
